...
export default function DesktopNav({
  ...
  return (
    <nav className="container mx-auto px-6">
      <div className="h-20 hidden xl:flex justify-between items-center relative gap-4">
        <div className="flex items-center gap-5">
          <button
            onClick={() => setBurgerOpen(!burgerOpen)}
            aria-label="Toggle burger menu"
            className="p-2 rounded-md hover:bg-gray-100 dark:hover:bg-slate-800 focus:outline-none transition-colors"
          >
            <div
          ...
          <Link href="/home">
            <span className="text-xl font-bold font-[bricle] text-vivid-red cursor-pointer">
          ...
          >
            <MapPinIcon className={`w-5 h-5 mr-2 ${isDark ? 'text-gray-200' : 'text-oxford-blue'}`} />
            <span className="text-sm font-medium truncate">{postcode || ''}</span>
            <ChevronDownIcon className={`w-4 h-4 ml-2 ${isDark ? 'text-gray-200' : 'text-oxford-blue'}`} />
          </button>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            ...
            className={`relative w-12 h-12 rounded-full border flex items-center justify-center hover:border-vivid-red hover:shadow-[0_0_10px_#ef4444] transition-all cursor-pointer ${
              isDark 
          ...
          {isLoggedIn ? (
            <button onClick={handleLogout} className="text-sm text-baltic-black dark:text-gray-200 font-medium hover:underline cursor-pointer transition-colors">
              {t('nav.logout')}
            </button>
          ) : (
            <>
