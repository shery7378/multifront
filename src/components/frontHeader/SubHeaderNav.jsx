            ...
            {navItems.map((item, index) => {
              ...
              if (item.label === 'Categories') {
                return (
                  ...
                  >
                    <button className="flex items-center gap-1.5 text-sm font-medium text-[#092E3B] hover:text-[#F44322] transition-colors py-4">
                      {item.label}
                    ...
                    <div className={`absolute top-full left-0 w-64 bg-white shadow-2xl rounded-b-xl border border-gray-100 z-[110] transition-all duration-300 transform origin-top ${categoriesOpen ? 'opacity-100 scale-y-100' : 'opacity-0 scale-y-0 pointer-events-none'}`}>
                      <div className="py-3 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        {loading && <div className="px-5 py-2 text-xs text-gray-400 italic">Finding categories...</div>}
                        {!loading && allCategories.map((cat) => (
                          <Link
                            key={cat.id}
                            href={`/products?category=${cat.id}`}
                            className="block px-5 py-2.5 text-sm text-gray-600 hover:bg-gray-50 hover:text-[#F44322] transition-colors"
              ...
              if (item.onClick) {
                return (
                  <button
                    ...
                    className="text-sm font-medium text-[#092E3B] hover:text-[#F44322] transition-colors"
              ...
              return (
                <Link
                  key={index}
                  href={item.href}
                  className={`text-sm font-medium transition-colors ${isActive ? 'text-[#F44322] font-semibold' : 'text-[#092E3B] hover:text-[#F44322]'
                    }`}
                  style={{ fontFamily: 'Manrope, sans-serif' }}
                >
                  {item.label}
          ...
          {navItems.map((item, index) => {
                ...
                return (
                    <div key={index} className="flex flex-col border-b border-gray-100">
                        <button 
                            ...
                            className="px-6 py-5 flex justify-between items-center text-sm font-medium text-[#092E3B] hover:bg-gray-50 transition-colors"
                        >
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ${categoriesOpen ? 'max-h-[50vh] opacity-100 py-2' : 'max-h-0 opacity-0'}`}>
                            {allCategories.map((cat) => (
                                <Link
                                    key={cat.id}
                                    href={`/products?category=${cat.id}`}
                                    className="block px-10 py-3 text-sm text-gray-600 hover:text-[#F44322]"
                                >
                                    {cat.name}
                                </Link>
                            ))}
            ...
            if (item.onClick) {
              return (
                <button
                  key={index}
                  onClick={item.onClick}
                  className="px-6 py-5 text-left text-sm font-medium text-[#092E3B] border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors"
                  style={{ fontFamily: 'Manrope, sans-serif' }}
                >
                  {item.label}
                </button>
            ...
            return (
              <Link
                key={index}
                href={item.href}
                className={`px-6 py-5 text-sm font-medium border-b border-gray-100 last:border-0 block transition-colors hover:bg-gray-50 ${isActive ? 'text-[#F44322] font-bold' : 'text-[#092E3B]'
                  }`}
                style={{ fontFamily: 'Manrope, sans-serif' }}
              >
                {item.label}
